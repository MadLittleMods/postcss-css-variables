...
var resolveValue = function(decl, map, /*optional*/ignorePseudoScope, /*internal debugging*/_debugIsInternal) {
	...
	resultantValue = resultantValue.replace(new RegExp(RE_VAR_FUNC.source, 'g'), function(match, variableName, fallback) {
		...
		// Check fallback is a variable or not
        if(fallback && fallback.indexOf('var') === 0) {
        	fallbackRoot = fallback.replace(new RegExp(RE_VAR_FUNC.source, 'g'), function(match, variableName) {
				...
				(map[variableName] || []).forEach(function(varDeclMapItem) {
				 ...
				 if(
						...
						fallbackRootValue = (varDeclMapItem || {}).calculatedInPlaceValue;
					}
				});
        	});
        }
        // if static value then set it as fallback value
        else if(fallback) {
            fallbackRootValue = fallback;
        }
        else {
        	// put var back as fallback
        	fallbackRootValue = fallback;
        }


		// Default to the calculatedInPlaceValue which might be a previous fallback, then try this declarations fallback
		var replaceValue = (matchingVarDeclMapItem || {}).calculatedInPlaceValue || fallbackRootValue;
